...
2. Audit current Render configuration, Docker build/runtime constraints, Node version, and the existing Codex CLI/SDK authentication method without changing secrets. Confirm that the production execution environment can run Codex with internet access restricted to only `target.com`, `www.target.com`, `kingsoopers.com`, and `www.kingsoopers.com`, with only `GET`, `HEAD`, and `OPTIONS` methods. A local logged-in CLI is useful only as a local development proof.
3. Prove with an isolated, non-product fixture/spike that the normal rendered websites can be navigated without calling a product API, a JSON/GraphQL endpoint, a third-party search engine, or any other host. Confirm the site UI can set/verify the exact store address and expose a user-visible product/search result. If a site needs a disallowed redirect, login, host, method, or direct API call to do this, treat the scope as unavailable rather than weakening it.
...
**Exit criteria:** The exact two-store/four-host boundary is encoded and technically enforceable; Codex has a documented production authentication/runtime path; and the service has defined limits and error behavior. If either site cannot be researched through its rendered pages inside that boundary, stop here and report the concrete limitation rather than adding a product API, a third-party site, an alternate store, or a local-only workaround.
...
2. Confirm the installed SDK’s actual supported thread/run and agent-internet options before coding them. Create a fresh thread per scan, run it server-side, use a read-only sandbox and approvals disabled, and enforce agent internet access with an empty baseline allowlist plus only `target.com`, `www.target.com`, `kingsoopers.com`, and `www.kingsoopers.com`, using only `GET`, `HEAD`, and `OPTIONS`. Do not rely on a prompt to enforce this boundary. If the SDK/runtime cannot enforce it, the feature remains unavailable.
3. Make the runtime use normal browser/page navigation and each site’s visible search/store-selection/product pages only. It must not run `curl`, `fetch`, scripts, direct HTTP calls, or API/JSON/GraphQL requests for product information; it must not use Google, Bing, a shopping aggregator, a map service, a CDN, login, cart, pickup, or checkout flow. A browser redirect or resource request to any host outside the four-host list terminates that site’s result as a domain-scope failure.
...
**Exit criteria:** The API can invoke a fake website researcher end-to-end, and the real SDK is isolated behind a tested, technically enforced four-host/read-only-web gate. No result from another site, another store, a direct product API, or unconfirmed rendered-page state can be applied.
