...
test('notification preferences can be synced and fetched by device token or device ID', async () => {
  ...
  assert.equal(tokenFetch.preferences.length, 15);
  assert.equal(
  ...
  assert.equal(
    tokenFetch.preferences.find((preference: { category: string }) => preference.category === 'laundry')
      .isEnabled,
    true,
  );
  assert.equal(
    tokenFetch.preferences.find((preference: { category: string }) => preference.category === 'freezer')
      .isEnabled,
    true,
  );
  assert.equal(
    tokenFetch.preferences.find((preference: { category: string }) => preference.category === 'refrigerator')
      .isEnabled,
    true,
  );
  assert.equal(
    tokenFetch.preferences.find((preference: { category: string }) => preference.category === 'thermostat_setpoint_high')
      .isEnabled,
    true,
  );
  assert.equal(
    tokenFetch.preferences.find((preference: { category: string }) => preference.category === 'weather_alerts')
      .isEnabled,
    true,
  );
  assert.equal(
